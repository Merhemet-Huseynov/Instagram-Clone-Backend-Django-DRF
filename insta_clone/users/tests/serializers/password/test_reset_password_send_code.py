...
def api_request() -> APIRequestFactory:
    """
    Creates a POST request using APIRequestFactory.
    """
...
def test_successful_password_change(user: User, api_request: APIRequestFactory) -> None:
    """
    Tests successful password change when the correct old password is provided.
    """
...
def test_invalid_old_password(user: User, api_request: APIRequestFactory) -> None:
    """
    Tests that an error is raised when an incorrect old password is provided.
    """
    api_request.user = user
    data = {
...
def test_password_mismatch(user: User, api_request: APIRequestFactory) -> None:
    """
    Tests that an error is raised when new_password and confirm_password do not match.
    """
    api_request.user = user
    data = {
        "old_password": "OldPassword123",
